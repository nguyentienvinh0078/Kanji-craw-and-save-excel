...
for course in courses:
    ...
    for row in range(x_offset_home_control, x_offset_home_control + total_row_home_control):
        ...
        for col in range(y_offset_home_control, y_offset_home_control + total_col_home_control):
            ...
            options = button_options(height=50, url=f'internal:{lesson_name}!A1', font_size=14)
    ...
    for lesson, word_list in input_data[course].items():
        ...
        for row in range(total_row):
            ...
            if row == 0:
                ...
                for col in range(len(sheet_data[row])):
                    ...
                    # ============== CONTENT =================
                        ...
                        worksheet.set_column(row, col, width=50)
                        worksheet.write_string(row, col, item, workbook.add_format({
                                'font': 'Times New Roman', 'font_size': 11,
                    ...
                    # ============== MAZZI SEARCH URL =================
                        ...
                        options = button_options(width=70, x_offset=10, y_offset=100, url=item)
                    ...
                    # ============== BUTTON ===========================
                        ...
                        options = button_options(width=70, x_offset=20, y_offset=100, url=f'internal:{home_sheet_name}!A1')
                        ...
                        lesson_back_id = f"Bài {int(lesson) - 1}" if first_condition else f"HOME"
                        back_url = f'internal:{int(lesson) - 1}!A1' if home_condition else f'internal:{home_sheet_name}!A1'
                        options = button_options(width=70, x_offset=10, y_offset=100, url=back_url)
                        worksheet.write_string(row+1, col, '', workbook.add_format({'border': False}))
                        worksheet.insert_textbox(row+1, col, lesson_back_id, options)
                        ...
                        options = button_options(width=70, x_offset=10, y_offset=100, url=netx_url)
                        worksheet.write_string(row+2, col, '', workbook.add_format({'border': False}))
                        worksheet.insert_textbox(row+2, col, lesson_go_id, options)
                    ...
                    #     worksheet.write_string(row+1, col, text_msg_cell, workbook.add_format({
                    #             'font': 'Times New Roman', 'font_size': 20, 'bold': True, 'color': '#00B050',
                    #             'text_wrap': True, 'align': 'center', 'valign': 'vcenter',
                    #         })
                    #     )
                    ...
                    #     worksheet.write_string(row+2, col, text_msg_cell, workbook.add_format({
                    #             'font': 'Times New Roman', 'font_size': 20, 'bold': True, 'color': '#00B050',
                    #             'text_wrap': True, 'align': 'center', 'valign': 'vcenter',
                    #         })
                    #     )
            ...
            button_middel_sheet_index = 3
            home_sheet_options = button_options(width=155, x_offset=10, y_offset=10, url=f'internal:{home_sheet_name}!A1')
            worksheet.insert_textbox(end_sheet_index, button_middel_sheet_index, home_sheet_name, home_sheet_options)
            
            top_sheet_options = button_options(width=155, x_offset=10, y_offset=10, url=f'internal:{int(lesson)}!A1')
            worksheet.insert_textbox(end_sheet_index, button_middel_sheet_index+1, f"ĐẦU TRANG", top_sheet_options)

            first_condition = lesson != f'{first_lesson + total_lesson - 1}'
            netx_url = f'internal:{int(lesson) + 1}!A1' if first_condition else f'internal:{home_sheet_name}!A1'
            options = button_options(width=155, x_offset=10, y_offset=10, url=netx_url)
            worksheet.insert_textbox(end_sheet_index, button_middel_sheet_index+2, f"SAU", options)

            first_condition = lesson != f'{first_lesson + total_lesson - 1}'
            lesson_go_id = f"Bài {int(lesson) + 1}" if first_condition else f"HOME"
            netx_url = f'internal:{int(lesson) + 1}!A1' if first_condition else f'internal:{home_sheet_name}!A1'
            options = button_options(width=155, x_offset=10, y_offset=10, url=netx_url)
            worksheet.insert_textbox(end_sheet_index, button_middel_sheet_index+2, lesson_go_id, options)
